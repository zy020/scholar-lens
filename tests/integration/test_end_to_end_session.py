...
Answer in Chinese with key English terms preserved inline:"""),
    ...
    @pytest.mark.asyncio
    async def test_pptx_end_to_end(self):
        import time, numpy as np
        from pathlib import Path
        from langchain_openai import ChatOpenAI, OpenAIEmbeddings
        from langchain_core.messages import HumanMessage, SystemMessage
        ...
        pptx_path = os.getenv("E2E_COURSEWARE_PPTX_PATH", "")
        if not pptx_path:
            pytest.skip("Set E2E_COURSEWARE_PPTX_PATH to run the courseware PPTX E2E test")
        if not Path(pptx_path).exists():
            pytest.skip(f"PPTX not found: {pptx_path}")

        s = Settings()
        llm = ChatOpenAI(api_key=s.llm.api_key, base_url=s.llm.base_url, model=s.llm.model)
        emb = OpenAIEmbeddings(api_key=s.embedding.api_key, base_url=s.embedding.base_url, model=s.embedding.model)

        print(f"\n=== E2E PPTX: {Path(pptx_path).name} ===")

        parser = PPTParser()
        start = time.time()
        doc = parser.parse(pptx_path)
        print(f"  Parse: {time.time()-start:.1f}s, {len(doc.pages)} slides, subtype={doc.doc_subtype}")

        assert doc.doc_subtype == "courseware_pptx"
        assert len(doc.pages) > 0

        chunker = SectionAwareChunker(max_chunk_tokens=800)
        chunks = chunker.chunk(doc, doc_id="ascend_tutorial")
        print(f"  Chunks: {len(chunks)}, tokens: {sum(estimate_tokens(c.text) for c in chunks)}")
        assert len(chunks) > 0
        ...
        pipeline = RerankerPipeline(diversity=True)
        fused = retriever.hybrid_search(query, q_emb, vr, top_k=5)
        reranked = pipeline.rerank(fused, query=query)

        print(f"  Top chunk: {reranked[0].text[:120]}...")
