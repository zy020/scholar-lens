...
- OCR/Vision 增强：GPU RapidOCR 可用时，上传后自动处理推荐 OCR 的页面；GPU OCR 不可用时，前端会提示使用 Vision 或配置 GPU OCR。
...
- 学习工具：流式 Chat、章节翻译、Study Brief 和 evidence 卡片；学习记忆作为后端个性化组件运行。
...
├── web/
│   └── src/          React/Vite 前端：侧边栏、阅读器、Chat、翻译、Study Brief 和配置面板
...
2. 等待文档状态变为 `ready`。
3. GPU OCR 可用时，质量诊断推荐的 OCR 页面会自动增强并应用；GPU OCR 不可用时，前端会提示使用 Vision 或配置 GPU OCR。需要更强增强时，可在 Config 中开启 Vision 和 LLM 质量评估。
4. 在 Reader 中选择章节。
...
9. 继续追问或切换学习任务；后端学习记忆会保持连续学习和个性化，不再作为独立前端面板展示。
