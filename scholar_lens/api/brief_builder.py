...
def build_unavailable_brief(
    ...
    *,
    ...
    reason = error or "学习简报需要配置并成功调用 LLM。"
    return PaperBriefResponse(
        ...
        problem="需要配置并成功调用 LLM 后才能生成学习简报。",
        motivation="系统不会使用规则摘要兜底，避免用浅层摘要误导学习。",
        limitations=[
            "未生成学习简报。",
            "请在配置面板启用可用 LLM 后重新生成学习简报。",
        ],
...
def build_not_generated_brief(
    ...
    *,
    ...
    return PaperBriefResponse(
        ...
        ocr_needed=ocr_needed,
        problem="学习简报尚未生成。",
        motivation="点击“生成学习简报”后，系统会使用已配置的 LLM 基于当前文档生成结构化学习材料。",
        limitations=["默认不会自动生成学习简报，避免上传后立即消耗模型调用。"],
        generated_at=datetime.utcnow().isoformat(timespec="seconds") + "Z",
    )
...
- important_slides: reason about which Slide/section ids deserve close review; mirror them into reading_focus.
