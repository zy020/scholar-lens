...
class VisionEnhancementExecutor:
        ...
        with tempfile.TemporaryDirectory(prefix="scholar_lens_vision_") as tmp:
            ...
            for page in pages:
                ...
                if not image_paths:
                    ...
                    continue
                try:
                    text = self._describe_images(page, image_paths)
    ...
    def _describe_images(self, page: int, image_paths: list[Path]) -> str:
        content: list[dict] = [{
            "type": "text",
            "text": (
                "Extract and explain the visible academic content on this page or slide. "
                "Preserve formulas, table values, labels, axes, and important English terms. "
